...
const EventCalendar: React.FC<EventCalendarProps> = ({ primaryDate, secondaryDate, countdownTarget }) => {
  const year = 2025;
  const month = 2; 
  ...
  const calculateTimeLeft = () => {
    const difference = new Date(countdownTarget).getTime() - new Date().getTime();
    if (difference > 0) {
      return {
  ...
  return (
    <section className="text-center py-9 px-7 w-full" data-aos="fade" data-aos-duration="1000">
      <div className="border-[5px] border-gray-200 py-5 px-4 rounded">
        <h2 className="text-lg font-dm-serif-display mb-4 font-semibold">THÁNG 3 - 2025</h2>
        ...
        <div className="grid grid-cols-4 gap-12 py-4 p-10 text-xl font-bold bg-[#ffe2f0] rounded-lg justify-center">
          <div className="flex flex-col items-center gap-2">
            <span className="text-xl font-dm-serif-display text-primary font-bold">{formatNumber(timeLeft.days)}</span>
